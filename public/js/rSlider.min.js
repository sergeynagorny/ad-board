!function () {
  ...
  t.prototype.init = function () {
    return typeof this.conf.target === `object` ? this.input = this.conf.target : this.input = document.getElementById(this.conf.target.replace(`#`, ``)), this.input ? (this.inputDisplay = getComputedStyle(this.input, null).display, this.input.style.display = `none`, this.valRange = !(this.conf.values instanceof Array), !this.valRange || this.conf.values.hasOwnProperty(`min`) && this.conf.values.hasOwnProperty(`max`) ? this.createSlider() : console.log(`Missing min or max value...`)) : console.log(`Cannot find target element...`);
    ...
    for (let e = 0, s = this.conf.values.length; e < s; e++) {
      let n = i(`span`);
      let l = i(`ins`);
      n.appendChild(l), this.scale.appendChild(n), n.style.width = e === s - 1 ? 0 : this.step + `px`, this.conf.labels ? l.innerHTML = this.conf.values[e] : e !== 0 && e !== s - 1 || (l.innerHTML = this.conf.values[e]), l.style.marginLeft = l.clientWidth / 2 * -1 + `px`;
    }
    return this.addEvents();
  }, t.prototype.updateScale = function () {
    this.step = this.sliderWidth / (this.conf.values.length - 1);
    for (let t = this.slider.querySelectorAll(`span`), i = 0, e = t.length; i < e; i++) {
      t[i].style.width = this.step + `px`;
    }
    return this.setValues();
  }, t.prototype.addEvents = function () {
    let t = this.slider.querySelectorAll(`.` + this.cls.pointer);
    let i = this.slider.querySelectorAll(`span`);
    ...
    if (t.preventDefault(), !this.conf.disabled) {
      ...
      return i === `left` && (this.activePointer = this.pointerL), i === `right` && (this.activePointer = this.pointerR), this.slider.classList.add(`sliding`);
    }
  }, t.prototype.move = function (t) {
    if (this.activePointer && !this.conf.disabled) {
      let i = (t.type === `touchmove` ? t.touches[0].clientX : t.pageX) - this.sliderLeft - this.pointerWidth / 2;
    ...
    return t && this.conf.values.indexOf(t) > -1 && (this.values[e] = this.conf.values.indexOf(t)), i && this.conf.values.indexOf(i) > -1 && (this.values.end = this.conf.values.indexOf(i)), this.conf.range && this.values.start > this.values.end && (this.values.start = this.values.end), this.pointerL.style.left = this.values[e] * this.step - this.pointerWidth / 2 + `px`, this.conf.range ? (this.conf.tooltip && (this.tipL.innerHTML = this.conf.values[this.values.start].toString().replace(/\B(?=(\d{3})+(?!\d))/g, `&thinsp;`), this.tipR.innerHTML = this.conf.values[this.values.end].toString().replace(/\B(?=(\d{3})+(?!\d))/g, `&thinsp;`)), this.input.value = this.conf.values[this.values.start] + `,` + this.conf.values[this.values.end], this.pointerR.style.left = this.values.end * this.step - this.pointerWidth / 2 + `px`) : (this.conf.tooltip && (this.tipL.innerHTML = this.conf.values[this.values.end]), this.input.value = this.conf.values[this.values.end]), this.values.end > this.conf.values.length - 1 && (this.values.end = this.conf.values.length - 1), this.values.start < 0 && (this.values.start = 0), this.selected.style.width = (this.values.end - this.values.start) * this.step + `px`, this.selected.style.left = this.values.start * this.step + `px`, this.onChange();
    ...
    this.timeout && clearTimeout(this.timeout), this.timeout = setTimeout(function () {
      if (t.conf.onChange && typeof t.conf.onChange === `function`) {
  ...
  var i = function (t, i, e) {
    ...
    return i && (s.className = i), e && e.length === 2 && s.setAttribute(`data-` + e[0], e[1]), s;
  };
  var e = function (t, i, e) {
    for (let s = i.split(` `), n = 0, l = s.length; n < l; n++) {
      t.addEventListener(s[n], e);
    }
  };
  var s = function (t) {
    let i = [];
    let e = t.values.max - t.values.min;
    if (!t.step) {
      return console.log(`No step defined...`), [t.values.min, t.values.max];
    }
    for (let s = 0, n = e / t.step; s < n; s++) {
